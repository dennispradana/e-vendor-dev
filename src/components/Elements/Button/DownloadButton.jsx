...
export const DownloadLainnya = ({ data }) => {
...
export const DownloadHarga = ({ data }) => {
  const { getFile, downloadFile } = fileService();
  const [files, setFiles] = useState([]);
  const [error, setError] = useState('');
  const [downloading, setDownloading] = useState(false);
    ...
    files.map((file, index) => (
      <div key={index}>
        <button
          ...
          className={`flex items-center px-4 mt-2 font-semibold text-sm text-blue-400 hover:text-blue-700 hover:underline gap-4 ${
            downloading ? 'cursor-not-allowed' : ''
          }`}
          disabled={downloading}
        >
...
export const DownloadTeknis = ({ data }) => {
  const { getFile, downloadFile } = fileService();
  const [files, setFiles] = useState([]);
  const [error, setError] = useState('');
  const [downloading, setDownloading] = useState(false);
