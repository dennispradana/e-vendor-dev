...
const FormPenawaranHarga = ({ dataHarga, formik, modal }) => {
  const { llsId } = useParams();
  const fileInputRef = useRef(null);
  const [data, setData] = useState([]);
  const { getTemplateHarga } = penyediaService();

  useEffect(() => {
    if (dataHarga && dataHarga.peserta && dataHarga.peserta.psr_dkh) {
  ...
  const handleChange = (rowIndex, field, value) => {
    ...
    formik.setFieldValue('peserta.psr_dkh', filteredData);
    formik.setFieldValue('peserta.psr_harga', total);
  };
  ...
  const handleExcelImport = (e) => {
    ...
    reader.onload = (event) => {
      ...
      setData(newRows);
      formik.setFieldValue('peserta.psr_dkh', newRows);
      formik.setFieldValue('peserta.psr_harga', jumlahTotal);
    };
  ...
  const handleDownload = async () => {
    try {
      const response = await getTemplateHarga(llsId);
  ...
  const RenderUploadFile = () => {
    return (
      <>
        {dataHarga.checklist?.harga?.map((item, index) => (
      ...
      <div className="relative overflow-x-auto">
        ...
        {formik.errors.paket?.pkt_hps && (
          <p className="mt-2 text-sm italic text-red-500 ">
            {formik.errors.peserta.psr_harga}
          </p>
        )}
      </div>
    </>
...
export default FormPenawaranHarga;
